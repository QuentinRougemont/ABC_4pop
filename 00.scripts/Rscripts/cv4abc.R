model_selection_abc_nnet <- function(target,x,sumstat,tol,gwt,noweight=F,rejmethod=F,nb.nnet=10,size.nnet=5, output="output", MaxNWts=2000)
{
require(nnet)
...
for(pseu in 1:nrow(target.s.tot)){
	...
	# wt1 defines the region we're interested in
		...
		ll<-NULL
	    	for (i in 1:nb.nnet)
	    	{
			fit1 <- nnet(scaled.sumstat[wt1,],class.ind(as.factor(x))[wt1,],weights=regwt,decay=the_decay[i],size=size.nnet,softmax=T,maxit=500,trace=F, MaxNWts=MaxNWts)
			ll<-c(ll,list(fit1))
		}
		vect_pred<-NULL
		for (i in 1:nb.nnet)
