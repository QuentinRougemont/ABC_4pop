...
In addition statistics associated to the Dfoil from [Pease & Hahn, 2015](https://academic.oup.com/sysbio/article/64/4/651/1650669) were computed, namely **Dfo**, **Dil**, **Dfi**, and **Dol** . 
...
| __D OL__            |  ABBAA-BABAA’s D statistics corresponding to P1/4 vs P2/P4         |
...
```
./00.scripts/models/model_parallel.sh [ID] [FOLDER] [model] [migration]   
Ex: ./00.scripts/models/model_parallel.sh 1 1 SC_2M_1N AC 
```
This example will run 400 multilocus simulations, with a secondary contact between population **A** and **C**.  
...
Then this can be automatize to run in parallel with 
```
./00.scripts/parallel_model_job_array.sh [model] [migration] 
```
This is designed to run on a slurm scheduler and will compute several simulation in **parallel**. The number of CPUs and array size must be adjusted to fit your cluster.
