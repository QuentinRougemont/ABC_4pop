...
###### DFOIL
def Dfoil(spA, spB, spC, spD):
    # spX is a list of haplotypes for the species X
    nSNP = len(spA) # number of SNPs
    ...
    for snp in range(nSNP):
        ...
        #Compute each ABBAA-BABAA
        nAAABA += (1-p1)*(1-p2)*(1-p3)*p4
        nAABBA += (1-p1)*(1-p2)*p3*p4
        nABBBA += (1-p1)*p2*p3*p4
        nBBBAA += p1*p2*p3*(1-p4)
        nBABAA += p1*(1-p2)*p3*(1-p4)
        nBABBA += p1*(1-p2)*p3*p4
        nBAABA += p1*(1-p2)*(1-p3)*p4
        nBBABA += p1*p2*(1-p3)*p4
        nBAAAA += p1*(1-p2)*(1-p3)*(1-p4)
        nABABA += (1-p1)*p2*(1-p3)*p4
        nABAAA += (1-p1)*p2*(1-p3)*(1-p4)
        nAABAA += (1-p1)*(1-p2)*p3*(1-p4)
        nABBAA += (1-p1)*p2*p3*(1-p4)
        nBBAAA += p1*p2*(1-p3)*(1-p4)
        
        #Dfoil
        Dfo_num = (nBABAA + nBBBAA + nABABA + nAAABA) - (nBAABA + nBBABA + nABBAA + nAABAA) 
        Dfo_den = (nBABAA + nBBBAA + nABABA + nAAABA) + (nBAABA + nBBABA + nABBAA + nAABAA)

        Dil_num = (nABBAA + nBBBAA + nBAABA + nAAABA) - (nABABA + nBBABA + nBABAA + nAABAA) 
        Dil_den = (nABBAA + nBBBAA + nBAABA + nAAABA) + (nABABA + nBBABA + nBABAA + nAABAA) 

        Dfi_num = (nBABAA + nBABBA + nABABA + nABAAA) - (nABBAA + nABBBA + nBAABA + nBAAAA) 
        Dfi_den = (nBABAA + nBABBA + nABABA + nABAAA) + (nABBAA + nABBBA + nBAABA + nBAAAA) 

        Dol_num = (nBAABA + nBABBA + nABBAA + nABAAA) - (nABABA + nABBBA + nBABAA + nBAAAA) 
        Dol_den = (nBAABA + nBABBA + nABBAA + nABAAA) - (nABABA + nABBBA + nBABAA + nBAAAA)  
        #V2:
        res = {}
        try:
            res["Dfo"] = Dfo_num/Dfo_den
        except:
            res["Dfo"] = -9
        try:
            res["Dil"] = Dil_num/Dil_den
        except:
            res["Dil"] = -9
        try:
            res["Dfi"] = Dfi_num/Dfi_den
        except:
            res["Dfi"] = -9
        try:
            res["Dol"] = Dol_num/Dol_den
        except:
            res["Dol"] = -9
        return(res)
...
res += 'D_fo_avg\tD_fo_std\t'
res += 'D_il_avg\tD_il_std\t'
res += 'D_fi_avg\tD_fi_std\t'
res += 'D_ol_avg\tD_ol_std\n'
outfile.write(res)
...
for line in sys.stdin: # read the ms's output from the stdin
    ...
    if test == 1:
        ...
        #           DtajD.append(tajD(tmpD['pi'], thetaD_locus, nSamD[nLoci_cnt - 1], tmp['sxD']+tmp['ss'], a1_spD[nLoci_cnt-1], a2_spD[nLoci_cnt-1]))
                    ...
                    Df_tmp = Dfoil(tmpA['freq'],tmpB['freq'],tmpC['freq'], tmpD['freq'])
                    D_fo.append(Df_tmp["Dfo"])
                    D_il.append(Df_tmp["Dil"])
                    D_ol.append(Df_tmp["Dol"])
                    D_fi.append(Df_tmp["Dfi"])
    ...
    if nLoci_cnt != 0 and len(sxA) == nLoci:
        ...
        D_fo_avg = cr_mean(D_fo)
        D_fo_std = cr_std(D_fo, D_fo_avg )
        D_il_avg = cr_mean(D_il)
        D_il_std = cr_std(D_il, D_il_avg)
        D_fi_avg = cr_mean(D_fi)
        D_fi_std = cr_std(D_fi, D_fi_avg)
        D_ol_avg = cr_mean(D_ol)
        D_ol_std = cr_std(D_ol, D_ol_avg)
        ...
        res += "{0:.5f}\t{1:.5f}\t".format(D_fo_avg, D_fo_std)
        res += "{0:.5f}\t{1:.5f}\t".format(D_il_avg, D_il_std)
        res += "{0:.5f}\t{1:.5f}\t".format(D_fi_avg, D_fi_std)
        res += "{0:.5f}\t{1:.5f}".format(D_ol_avg, D_ol_std)
